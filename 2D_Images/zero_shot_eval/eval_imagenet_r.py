...
parser = argparse.ArgumentParser(description='PyTorch ImageNet_r eval')
...
log_path = os.path.join(checkpoint, config["models"]["arch"], config["models"]["dropout_types"], prob_dir, "imagenet_r.log")
...
def show_calibration_results(confidence, correct):
    ...
    logging.info('RMS Calib Error (%): \t{:.2f}'.format(
        100 * calib_err(confidence, correct, p='2')))
    
    logging.info('AURRA (%): \t\t{:.2f}'.format(
...
imagenet_r_folder = str(config["imagenet_r_path"])
print(imagenet_r_folder)
# os.chmod(imagenet_r_folder, 0o777)
...
imagenet_r = dset.ImageFolder(root="imagenet_r_folder", transform=test_transform)
imagenet_r_loader = torch.utils.data.DataLoader(imagenet_r, batch_size=64, shuffle=False,
                                         num_workers=4, pin_memory=True)

def create_symlinks_to_imagenet(imagenet_folder, validation_dataset_path, imagenet_r_classes):
    if not os.path.exists(imagenet_folder):
        os.makedirs(imagenet_folder, mode=0o777)
    
        for folder in imagenet_r_classes:
...
imagenet_200_folder_temp = str(config["imagenet_200_path_temp"])
imagenet_200_folder = os.path.join(imagenet_200_folder_temp, str(config["host_device"]))
if not os.path.exists(imagenet_200_folder):
    os.makedirs(imagenet_200_folder)
print(imagenet_200_folder)
# os.chmod(imagenet_200_folder, 0o777)

create_symlinks_to_imagenet(imagenet_200_folder, config["imagenet_validation_path"], imagenet_r_wnids)

iid_examples = dset.ImageFolder(root=imagenet_200_folder, transform=test_transform)
iid_loader = torch.utils.data.DataLoader(iid_examples, batch_size=64, shuffle=False,
                                         num_workers=4, pin_memory=True)

# model and optimizer
net = create_model(str(args.arch), pretrained=True, num_classes=1000, in_chans=3, global_pool='avg')
...
def get_acc(loader):
    ...
    logger.info(f'Accuracy (%):\t\t {round(100*acc, 2)}')
...
logger.info("\n")
logger.info('ImageNet-200 Results')
