...
parser = argparse.ArgumentParser(description='PyTorch ImageNet_adv eval')
...
# set directories
prob_dir = 'prob_' + str(config["models"]["prob"]) + '_report'

# if args.random_dropout_block:
#     block_folder = "Random_Erasing_Block"
# else:
#     block_folder = "Relavance_Erasing_Block"

checkpoint=str(config["model_checkpoint_root"])
...
log_path = os.path.join(checkpoint, config["models"]["arch"], config["models"]["dropout_types"], prob_dir, "imagenet_adv.log")
...
PATH_TO_IMAGENET_A = str(config["imagenet_a_path"])
PATH_TO_IMAGENET_O = str(config["imagenet_o_path"])
PATH_TO_IMAGENET_VAL = str(config['imagenet_validation_path'])
...
imagenet_o_folder_temp = r"/tmp/imagenet_val_for_imagenet_o_odd"
imagenet_val_for_imagenet_o_odd = os.path.join(imagenet_o_folder_temp, str(config["host_device"]))


def create_symlinks_to_imagenet(imagenet_folder, validation_dataset_path, folder_to_scan):
    if not os.path.exists(imagenet_folder):
        os.makedirs(imagenet_folder, mode=0o777)

        folders_of_interest = os.listdir(folder_to_scan)
        for folder in folders_of_interest:
            source_path = os.path.join(validation_dataset_path, folder)
            target_path = os.path.join(imagenet_folder, folder)

            if os.path.exists(source_path):
                os.symlink(source_path, target_path, target_is_directory=True)
            else:
                logger.info(f"Warning: Source folder does not exist - {source_path}")

    else:
        # print('Folder containing IID validation images already exists')
        logger.info('Folder containing IID validation images already exists')

create_symlinks_to_imagenet(imagenet_val_for_imagenet_o_odd, PATH_TO_IMAGENET_VAL, PATH_TO_IMAGENET_O)

val_examples_imagenet_o = dset.ImageFolder(root=imagenet_val_for_imagenet_o_odd, transform=test_transform)
...
def get_imagenet_a_results(loader, net, mask):
    ...
    logger.info(f'Accuracy (%):, {round(100*acc, 4)}')
...
def print_measures_old(auroc, aupr):
...
def get_imagenet_o_results(in_loader, out_loader, net, mask):
    ...
    aurocs = measures[0]; auprs = measures[1]; 

    print_measures_old(aurocs, auprs)

# model and optimizer
net = create_model(str(config["models"]["arch"]), pretrained=True, num_classes=1000, in_chans=3, global_pool='avg')
net = torch.nn.DataParallel(net, device_ids=[0]).cuda()

final_path = os.path.join(checkpoint, config["models"]["arch"], config["models"]["dropout_types"], prob_dir, "model_best.pth.tar")

pretrained = torch.load(final_path, map_location=device)

best_acc = pretrained['best_acc']
net.load_state_dict(pretrained['state_dict'])

net = net.to(device)
net.eval()
    
logger.info(f"ImageNet-1k test accuracy {best_acc}")
logger.info("\n")
logger.info("ImageNet-A Results")
get_imagenet_a_results(nae_loader, net=net, mask=imagenet_a_mask)
logger.info("\n")
logger.info("ImageNet-O Results")
